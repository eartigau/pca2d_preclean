...
- **A data root.** The objects it holds appear in the list, each with how many
  spectra it has, **its median SNR**, **its median exposure time** and the
  instrument that took them. Nothing is ever written in the data root: it can
  be a read-only archive or the shared disk.
...
per-order extraction SNR APERO wrote (`EXTSNxxx`), `EXPTIME`, and `INSTRUME`,
which is read from the FILE and never from a configuration. The answers go into
...
unchanged, and only what was added or replaced is read. **Rescan** is that same
