...
residuals dominate, the velocity scatter falls from 47.7 to 20.4~m\,s$^{-1}$.
On three NIRPS campaigns whose delivered velocities are already good, the same
correction is neutral at best. A pair of twin experiments, two 14-night slices
...
\toprule
Target & Instrument & $T_{\rm eff}$ (K) & exposures & nights & delivered rms (m\,s$^{-1}$)\\
\midrule
TOI-2120 & SPIRou & 3179 & 321 & --- & 47.7\\
Proxima  & NIRPS  & 3050 & 782 & 259 & 2.9\\
GJ~1     & NIRPS  & 3478 & 292 & 146 & 2.7\\
TOI-4552 & NIRPS  & 3304 & 119 & 60  & 12.6\\
\bottomrule
\end{tabular}
\caption{The campaigns. Nights are the nightly coadds the NIRPS fits use; the
delivered rms is the line-by-line velocity scatter of the spectra as reduced,
with no precleaning.}
\end{table}
...
Target & rms & robust $\sigma$ & rms & robust $\sigma$\\
\midrule
TOI-2120 & 47.71 & 34.99 & 20.35 & 16.16\\
Proxima  &  2.86 &  2.22 &  3.04 &  2.52\\
GJ~1     &  2.70 &  2.74 &  2.85 &  2.78\\
TOI-4552 & 12.64 & 11.37 & 15.35 & 14.56\\
\bottomrule
\end{tabular}
\caption{Line-by-line velocities, in m\,s$^{-1}$, on the exposures both series
share. The correction transforms the campaign whose residuals are telluric,
and costs where the delivered velocities are already good.}
\label{tab:overall}
\end{table}

The pattern of Table~\ref{tab:overall} is not a signal-to-noise effect: GJ~1 is
four magnitudes brighter than TOI-4552 and both are NIRPS campaigns stacked by
night, yet one is neutral and the other loses 3~m\,s$^{-1}$.
...
behind, and it is not free where they are not. On TOI-2120 it removes more than
half the scatter. On Proxima and GJ~1 the delivered velocities are already at
2.7 to 2.9~m\,s$^{-1}$, and the correction adds 0.2. Applying it there buys
nothing, and the decision can be made in advance from the barycentric span and
from how much of the scatter the formal errors already explain.
...
removes more than half of the velocity scatter of a campaign dominated by
telluric residuals.
...
\item The star's mean spectrum must carry a coefficient of exactly one. The
freedom given to a star component is used to absorb an annual, barycentric
mode, and removing it recovers most of the damage on the worst case.
